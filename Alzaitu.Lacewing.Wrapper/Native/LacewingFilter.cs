...
namespace Alzaitu.Lacewing.Wrapper.Native
{
    internal unsafe struct LacewingFilter : IDisposable
    {
        ...
        public ref LacewingFilter Clone() => ref *lw_filter_clone(ref this);

        public void Dispose() => lw_filter_delete(ref this);

        public static ref LacewingFilter Create() => ref lw_filter_new();

        #region Native

        [DllImport("liblacewing.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
        private static extern ref LacewingFilter lw_filter_new();
