...
namespace Alzaitu.Lacewing.Wrapper.Native
{
    internal unsafe struct LacewingAddress : IDisposable
    {
        ...
        public LacewingError* Resolve() => lw_addr_resolve(ref this);

        public LacewingAddress* Clone() => lw_addr_clone(ref this);

        public override string ToString() => Marshal.PtrToStringUTF8(lw_addr_tostring(ref this));
        ...
        public static LacewingAddress* Create(string hostname, string service) => lw_addr_new(hostname, service);

        public static LacewingAddress* Create(string hostname, long port) => lw_addr_new_port(hostname, port);

        public static LacewingAddress* Create(string hostname, string service, AddressHint hints) => lw_addr_new_hint(hostname, service, hints);

        public static LacewingAddress* Create(string hostname, long port, AddressHint hints) => lw_addr_new_port_hint(hostname, port, hints);

        #region Native

        [DllImport("liblacewing.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        private static extern LacewingAddress* lw_addr_new(string hostname, string service);

        [DllImport("liblacewing.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        private static extern LacewingAddress* lw_addr_new_port(string hostname, long port);

        [DllImport("liblacewing.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        private static extern LacewingAddress* lw_addr_new_hint(string hostname, string service, AddressHint hints);

        [DllImport("liblacewing.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        private static extern LacewingAddress* lw_addr_new_port_hint(string hostname, long port, AddressHint hints);
        ...
        [DllImport("liblacewing.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        private static extern IntPtr lw_addr_tostring(ref LacewingAddress address);
